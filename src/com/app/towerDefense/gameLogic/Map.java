...
public class Map {
	...
	public String mapValidations(MapModel new_mapModel) {
		...
		if (!new_mapModel.isEntryDone || new_mapModel.getEntryPoint() == null) {
			...
			new_mapModel.setMapAutoGeneratedDescription(routeStatus);
				...
				|| new_mapModel.getExitPoint() == null) {
	...
	public String mapPathValidation(MapModel new_map) {
		...
		while ((i >= 0 && i < mapHeight) || (j >= 0 && j < mapWidth)) {
			...
			if (i < mapHeight
				...
				if (i == mapHeight - 1) {
					...
					if (cellValue == ApplicationStatics.MAP_PATH_POINT) {
						...
						if (!ApplicationStatics.MAP_ROUT_PATH.contains("" + i
								+ "," + j + ";")) {
							ApplicationStatics.MAP_ROUT_PATH += "" + i + ","
									+ j + ";";
						}
			...
			if (i >= 0
					&& (dirUpDown == E_MapValidationDirecton.Initial || dirUpDown == E_MapValidationDirecton.Up)) {
				if (i == 0) {
					dirUpDown = E_MapValidationDirecton.Down;
				} else {
					cellValue = new_map.getMapGridSelection()[i - 1][j];
			...
			if (j < mapWidth
					&& (dirLeftRight == E_MapValidationDirecton.Initial || dirLeftRight == E_MapValidationDirecton.Left)) {
				if (j == mapWidth - 1) {
			...
			if (j >= 0
					&& (dirLeftRight == E_MapValidationDirecton.Initial || dirLeftRight == E_MapValidationDirecton.Right)) {
				if (j == 0) {
					dirLeftRight = E_MapValidationDirecton.Left;
				} else {
					cellValue = new_map.getMapGridSelection()[i][j - 1];
		...
		if (status.contains("Success")) {
			...
			if (!ApplicationStatics.MAP_ROUT_PATH.contains(""
					+ new_map.getExitPoint().x + "," + new_map.getExitPoint().y
					+ ";")) {
				ApplicationStatics.MAP_ROUT_PATH += ""
						+ new_map.getExitPoint().x + ","
						+ new_map.getExitPoint().y + ";";
			}
